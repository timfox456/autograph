...
def test_identity_matcher_performance(dataset: pd.DataFrame, tmp_path):
    ...
    train_df, test_df = train_test_split(
        dataset, test_size=0.2, random_state=42, stratify=dataset["identity"]
    )
...
def test_consistency_checker_performance(dataset, tmp_path):
    ...
    for _, row in test_df.iterrows():
        ...
        if pred is not None:
            results.append({"identity": identity, "is_anomaly": pred == -1, "type": "positive"})
    ...
    for _, row in test_df.sample(n=min(20, len(test_df)), random_state=42).iterrows():
        ...
        if pred is not None:
            results.append({"identity": wrong_id, "is_anomaly": pred == -1, "type": "negative"})
