...
def test_engine_consistency_missing_interface_methods():
    class ValidMatcher:
        def is_trained(self): return True
        def predict_probs(self, X): return [("alice", 1.0)]
...
def test_engine_is_trained_implementation_exception_handling():
    ...
    class Wrapper:
        def __init__(self, impl):
            self.implementation = impl
        features = ["a"]
        def predict(self, X): return [("alice", 1.0)]
        def score(self, id, X): return 1, 1.0

    # This should hit the 'except Exception' in the implementation check
    engine = AttestationEngine(matcher_impl=Wrapper(BrokenImpl()), consistency_impl=Wrapper(BrokenImpl()))
...
def test_engine_heuristic_ai_leak():
    ...
    class MockMatcher:
        def is_trained(self): return True
        def predict_probs(self, X): return [("user_alice", 0.9)]
