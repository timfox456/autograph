    ...
    def extract(self, code: str, root_node: Node) -> Dict[str, Any]:
        ...
        def traverse(node: Node):
            ...
            # or the first child of the module itself.
                ...
                node.type == "string" and node.parent and node.parent.type == "expression_statement"
            ):
        ...
        comment_texts = [
            c.text.decode("utf-8", errors="ignore").strip("#").strip() for c in comments
        ]
        ...
        debt_markers_count = sum(
            1 for t in comment_texts if any(m in t.upper() for m in ["TODO", "FIXME", "HACK"])
        )
