...
class StringContentExtractor:
    ...
    ERROR_VERBS = [
        'failed', 'could not', 'cannot', "can't", 'unable to', 'error', 'invalid',
        'missing', 'not found', 'does not exist', "doesn't exist", 'unsupported',
        'unexpected', 'wrong', 'bad', 'corrupt', 'broken', 'timeout', 'refused',
        'denied', 'forbidden', 'unauthorized', 'not allowed', 'abort', 'fatal'
    ]
    ...
    def _analyze_placeholders(self, strings: List[str], total: int) -> Dict[str, Any]:
        ...
        if placeholder_counts:
            ...
            features['dominant_placeholder_style'] = list(self.placeholder_patterns.keys()).index(dominant)
        else:
            features['dominant_placeholder_style'] = -1
        
        # Placeholder naming diversity
        total_placeholders = sum(placeholder_counts.values())
        features['placeholder_diversity'] = len(placeholder_counts) / len(self.placeholder_patterns) if self.placeholder_patterns else 0
