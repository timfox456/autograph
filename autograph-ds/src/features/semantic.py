...
LIBRARY_DOMAINS = {
    ...
    "serialization": ["json", "pickle", "yaml", "toml", "msgpack", "protobuf", "avro", "xml"],
    "crypto_security": ["hashlib", "secrets", "cryptography", "bcrypt", "jwt", "openssl", "pynacl"],
    "text_processing": [
        "re",
        "string",
        "textwrap",
...
class SemanticFingerprintExtractor:
    ...
    def _extract_api_calls(self, code: str, root_node: Node) -> Dict[str, Any]:
        ...
        features["call_diversity"] = (
            len(set(function_calls)) / total_calls if total_calls > 0 else 0
        )
